        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '进货单名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jinhuodanName" placeholder="进货单名称" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '进货类型' : ''">
                        <el-select v-model="searchForm.jinhuodanTypes" placeholder="请选择进货类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in jinhuodanTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jinhuodan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gongyingshang/upload/jinhuodanMuBan.xls"
                        >批量导入商品进货数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jinhuodan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jinhuodanUploadSuccess"
                                :on-error="jinhuodanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jinhuodan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入商品进货数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jinhuodan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jinhuodan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jinhuodanUuidNumber"
                                   header-align="center"
                                   label="进货单编号">
                        <template slot-scope="scope">
                            {{scope.row.jinhuodanUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="进货单名称">
                        <template slot-scope="scope">
                            {{scope.row.jinhuodanName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jinhuodanFile"
                                     header-align="center"
                                     width="200"
                                     label="进货单">
                        <template slot-scope="scope">
                            <div v-if="scope.row.jinhuodanFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.jinhuodanFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jinhuodan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jinhuodan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jinhuodan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              jinhuodanTypesSelectSearch : [],
            form:{
                id : null,
                yuangongId : null,
                gongyingshangId : null,
                jinhuodanUuidNumber : null,
                jinhuodanName : null,
                jinhuodanFile : null,
                jinhuodanTypes : null,
                caozuoTime : null,
                jinhuodanContent : null,
                jinhuodanBeizhuContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jinhuodan',//当前表表名,
                        sumColum : 'jinhuodan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jinhuodan_name',//分组字符串字段
                        // types : 'jinhuodan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.jinhuodanName!= '' && this.searchForm.jinhuodanName!= undefined) {
                    params['jinhuodanName'] = '%' + this.searchForm.jinhuodanName + '%'
                }
                 
                if (this.searchForm.jinhuodanTypes!= '' && this.searchForm.jinhuodanTypes!= undefined) {
                    params['jinhuodanTypes'] = this.searchForm.jinhuodanTypes
                }
                                        
                params['jinhuodanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=jinhuodan_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.jinhuodanTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            jinhuodanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jinhuodan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
